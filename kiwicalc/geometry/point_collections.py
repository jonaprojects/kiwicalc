...
from kiwicalc.geometry.points import Point, Point1D, Point2D, Point3D, Point4D
...
class PointCollection:
    ...
    def __init__(self, points: Iterable=()):
        ...
        for point in points:
            ...
            if isinstance(point, Point):
                ...
                raise TypeError(f"encountered invalid type '{type(point)}' of value {point} when creating a PointCollection object. ")
    ...
    def longest_distance(self, get_points=False):
        """ Gets the longest distance between two dots in the collection"""
    ...
    def scatter(self, show=True):
        ...
        if dimensions == 1:
            ...
            plt.hlines(0, min_value, max_value)
            plt.xlim(0.8 * min_value, 1.2 * max_value)
            plt.ylim(-1, 1)
            y = np.zeros(len(self._points))
...
class Point2DCollection(PointCollection):
    ...
    def plot_regression(self, show=True):
        if len(self._points[0].coordinates) == 2:
            linear_function = self.linear_regression()
