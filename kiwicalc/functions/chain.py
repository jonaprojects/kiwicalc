...
from kiwicalc.functions.function import Function, FunctionCollection
...
class FunctionChain(FunctionCollection):
    ...
    def plot(self, start: float=-10, stop: float=10, step: float=0.01, ymin: float=-10, ymax: float=10, title=None, show_axis=True, show=True, fig=None, ax=None, formatText=False, values=None):
        ...
        if num_of_variables == 0:
            ...
            plot_function(self.execute_all, start=start, stop=stop, step=step, ymin=ymin, ymax=ymax, title=title, show_axis=show_axis, show=show, fig=fig, ax=ax, formatText=formatText, values=values)
        elif num_of_variables == 2:
            return plot_function_3d(self.execute_all, start=start, stop=stop, step=step, ax=ax, fig=fig, meshgrid=values)
        else:
            raise ValueError(f'Can only plot functions with one or two variables: found ')

    def scatter(self, start: float=-10, stop: float=10, step: float=0.01, ymin: float=-10, ymax: float=10, title=None, show_axis=True, show=True, fig=None, ax=None, values=None):
