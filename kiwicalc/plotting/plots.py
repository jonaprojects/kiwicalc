...
def plot_function_3d(given_function: 'Union[Callable, str, IExpression]', start: float=-3, stop: float=3, step: float=0.3, xlabel: str='X Values', ylabel: str='Y Values', zlabel: str='Z Values', show=True, fig=None, ax=None, write_labels=True, meshgrid=None):
    if step < 0.1:
        step = 0.3
...
def plot_functions_3d(functions: 'Iterable[Union[Callable, str, IExpression]]', start: float=-5, stop: float=5, step: float=0.1, xlabel: str='X Values', ylabel: str='Y Values', zlabel: str='Z Values'):
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    x = y = np.arange(start, stop, step)
...
def plot_vector_2d(x_start: float, y_start: float, x_distance: float, y_distance: float, show=True, fig=None, ax=None):
    ...
    ax.arrow(x_start, y_start, x_distance, y_distance, head_width=0.1, width=0.01)
    if show:
        plt.show()
...
class Graph2D(Graph):
    ...
    def plot(self, start: float=-10, stop: float=10, step: float=0.01, ymin: float=-10, ymax: float=10, text=None, show_axis=True, show=True, formatText=False, values=None):
        if values is None:
        ...
        for obj in self._items:
            if isinstance(obj, (IExpression, Function)):
                obj.plot(start=start, stop=stop, step=step, ymin=ymin, ymax=ymax, text=graph_title, show_axis=show_axis, fig=self._fig, ax=self._ax, show=False, formatText=formatText, values=values)
...
class Graph3D(Graph):
    ...
    def plot(self, functions: Iterable[Union[Callable, str, IExpression]], start: float=-5, stop: float=5, step: float=0.1, xlabel: str='X Values', ylabel: str='Y Values', zlabel: str='Z Values'):
        return plot_functions_3d(functions=functions, start=start, stop=stop, step=step, xlabel=xlabel, ylabel=ylabel, zlabel=zlabel)

    def scatter(self, functions: Iterable[Union[Callable, str, IExpression]], start: float=-5, stop: float=5, step: float=0.1, xlabel: str='X Values', ylabel: str='Y Values', zlabel: str='Z Values'):
        return scatter_functions_3d(functions=functions, start=start, stop=stop, step=step, xlabel=xlabel, ylabel=ylabel, zlabel=zlabel)
