...
from typing import Union, Tuple, List, Optional, Any, Dict, Set
from anytree import Node, RenderTree, AsciiStyle, PreOrderIter, ZigZagGroupIter
try:
...
class Occurrence:
    ...
    def chance(self, chance: float):
        if 0 <= chance <= 1:
            ...
            warnings.warn(f'Occurrence._chance(setter): failed to set since  the probability must be in the range 0 to 1, got {chance} ')
...
class ProbabilityTree:
        ...
        for node_dict in tree_values[1:]:
            ...
            if parent_name is None:
                ...
                if parent_node is None:
                    warnings.warn(f"ProbabilityTree.tree_from_json(): couldn't find parent '{parent_name}.{node_dict['_identifier']} will be lost unless it is resolved.'")
            new_node = Node(name=Occurrence(node_dict['_chance'], identifier=node_dict['_identifier']), parent=parent_node)
            ...
            if parent_name.lower().strip() in ('', 'none') or parent_name is None:
                ...
                for existing_node in nodes:
                    if existing_node.name._identifier == parent_name:
                        ...
                        break
                if parent_node is None:
                    warnings.warn(f"ProbabilityTree.tree_from_xml(): couldn't find parent '{parent_name}.{name} will be lost unless it is resolved.'")
    ...
    def to_dict(self):
        ...
        for level_nodes in nodes:
            for node in level_nodes:
                new_dict[node.name.identifier] = {'parent': node.parent, '_identifier': node.name.identifier, '_chance': node.name.chance}
        return new_dict
    ...
    def __to_dict_json(tree):
        ...
        for level_nodes in nodes:
            for node in level_nodes:
                new_dict[node.name._identifier] = {'parent': node.parent.name._identifier if node.parent else None, '_identifier': node.name._identifier, '_chance': node.name._chance}
        return new_dict
    ...
    def add(self, probability: float, identifier: str, parent=None):
        ...
        if level_sum > 1:
            ...
            warnings.warn(f'ProbabilityTree.add_occurrence(): Probability sum in depth {depth} (sum is {level_sum})) is bigger than 1, expected 1 or less.')
        return node
